...
void USBH_USR_EnumerationDone(void)
{
  ...
  lcd_display_string_line( LCD_PIXEL_HEIGHT - 42, "To start the HID class operations: " );
...
void USR_MOUSE_Init	(void)
{
  ...
  lcd_dispaly_char(MOUSE_WINDOW_X + 1,
                            MOUSE_WINDOW_Y + 1,
                            'x');
  x_loc  = 0;
  y_loc  = 0; 
...
void  USR_KEYBRD_ProcessData (uint8_t data)
{
  ...
  else if (data == '\r')
  {
    ...
    lcd_dispaly_char(KeybrdCharXpos,KeybrdCharYpos, ' '); 
  }
  else
  {
    lcd_dispaly_char(KeybrdCharXpos,KeybrdCharYpos, data);    
