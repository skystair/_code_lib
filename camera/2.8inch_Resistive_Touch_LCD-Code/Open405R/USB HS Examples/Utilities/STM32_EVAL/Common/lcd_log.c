...
void LCD_LOG_SetHeader (uint8_t *Title)
{
  ...
  lcd_clear_line(0);
  lcd_display_string_line(cFont->Height, tmp);
  lcd_clear_line(2 * cFont->Height);

  lcd_set_back_color(Black);
  lcd_set_font (&Font8x12);
}
...
void LCD_LOG_SetFooter(uint8_t *Status)
{
  ...
  lcd_display_string_line(LCD_PIXEL_HEIGHT - cFont->Height, tmp);
  lcd_display_string_line(LCD_PIXEL_HEIGHT - cFont->Height, Status);
  lcd_set_back_color(Black);  
}
...
{
  uint8_t i=0;
  sFONT *cFont = lcd_get_font();
  
  for (i= 0 ; i < YWINDOW_SIZE; i++)
  {
    lcd_clear_line((i + YWINDOW_MIN) * cFont->Height);
  }
...
static void LCD_LOG_UpdateDisplay (void)
{
  ...
  {
    lcd_set_text_color(LCD_CacheBuffer[cnt + LCD_CacheBuffer_yptr_bottom].color);
    lcd_display_string_line ((YWINDOW_MIN + LCD_CacheBuffer_yptr_bottom) * cFont->Height,
  ...
  else
  {
    ...
    for  (cnt = 0 ; cnt < YWINDOW_SIZE ; cnt ++)
    {
      ...
      lcd_set_text_color(LCD_CacheBuffer[index].color);
      lcd_display_string_line ((cnt + YWINDOW_MIN) * cFont->Height, 
