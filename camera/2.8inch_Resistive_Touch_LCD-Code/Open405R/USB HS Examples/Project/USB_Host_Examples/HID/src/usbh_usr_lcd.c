...
{ 
  ...
  if ((x != 0) || (y != 0)) 
  {
    ...
    {
      lcd_display_char(MOUSE_WINDOW_X + prev_x ,\
        MOUSE_WINDOW_Y + prev_y, ' ');
    }
    else if((prev_x == 0) && ( prev_y == 0))
    {
      
      lcd_display_char(MOUSE_WINDOW_X + 2,
                                MOUSE_WINDOW_Y + 1,
                                ' ');    
    }
    
    lcd_display_char  (MOUSE_WINDOW_X + x_loc ,\
...
/* Set the color for button press status */
  lcd_set_text_color(LCD_COLOR_GREEN);
  lcd_set_back_color(LCD_COLOR_GREEN);
  ...
  switch (button_idx)
  {
    /* Left Button Pressed */
  case 0 :
    lcd_draw_full_rect(HID_MOUSE_BUTTON1_XCHORD, HID_MOUSE_BUTTON_YCHORD,\
  ...
  case 1 :
    lcd_draw_full_rect(HID_MOUSE_BUTTON2_XCHORD,HID_MOUSE_BUTTON_YCHORD,\
       HID_MOUSE_BUTTON_WIDTH,HID_MOUSE_BUTTON_HEIGHT);
    break; 
    
    /* Middle button Pressed */  
  case 2 :
    lcd_draw_full_rect(HID_MOUSE_BUTTON3_XCHORD,HID_MOUSE_BUTTON_YCHORD,\
      HID_MOUSE_BUTTON_WIDTH,HID_MOUSE_BUTTON_HEIGHT);
    break;
...
{  
 /* Set the color for release status */
  ...
  lcd_set_back_color(LCD_COLOR_WHITE);
