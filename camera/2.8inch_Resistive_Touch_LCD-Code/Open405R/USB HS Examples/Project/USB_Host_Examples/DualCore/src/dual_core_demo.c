...
static void Demo_SelectItem (uint8_t **menu , uint8_t item)
{
  ...
  {
  case 0 : 
    lcd_set_back_color(Red);
    lcd_display_string_line( LINE_01 ,menu [0]);
    lcd_set_back_color(Blue);    
    lcd_display_string_line( LINE_02, menu [1]);
    lcd_display_string_line( LINE_03, menu [2]);
  ...
  case 3 : 
    lcd_set_back_color(Blue);
    lcd_display_string_line( LINE_01 ,menu [1]);
    lcd_set_back_color(Blue);    
    lcd_display_string_line( LINE_02, menu [2]);
    lcd_set_back_color(Red);  
    lcd_display_string_line( LINE_03, menu [3]); 
  ...
  if (res == FR_OK) {
    for (;;) {
      ...
      if(line_idx > 12)
      {
        line_idx = 0;
        wait_user_input = 1;
        lcd_set_text_color(Green);
        lcd_display_string_line( LCD_PIXEL_HEIGHT - 48, "Press any key to continue...");
        lcd_set_text_color(LCD_LOG_DEFAULT_COLOR); 
...
//        STM_EVAL_LEDOff(LED4);
        lcd_clear_line( LCD_PIXEL_HEIGHT - 48);
      } 
      wait_user_input = 0; 
...
static void Show_Image(void)
{
  ...
  lcd_write_command(0x02, 0 >> 8);
	lcd_write_command(0x03, 0 & 0xFF); //Column Start
	lcd_write_command(0x06, 0 >> 8);
	lcd_write_command(0x07, 0 & 0xFF); //Row Start
	lcd_write_command(0x04, 0xef >> 8);
	lcd_write_command(0x05, 0xef & 0xFF); //Column End
	lcd_write_command(0x08, 0x013f >> 8);
	lcd_write_command(0x09, 0x013f & 0xFF); //Row End
	lcd_write_command(0x0A, 0 >> 8);
	lcd_write_command(0x0B, 0 & 0xFF); //Column End
	lcd_write_command(0x0C, 0  >> 8);
	lcd_write_command(0x0D, 0  & 0xFF); //Row End
	...
	lcd_write_byte(0,LCD_DATA);
	lcd_write_byte(0,LCD_DATA);
	lcd_write_byte(0xef,LCD_DATA);
	lcd_write_byte(0x2B,LCD_CMD);
	lcd_write_byte(0,LCD_DATA);
	lcd_write_byte(0,LCD_DATA);
	lcd_write_byte(0x013f >> 8,LCD_DATA);
	lcd_write_byte(0x013f & 0xFF,LCD_DATA);
    ...
    for (;;) {
      ...
      else 
      {
        if((strstr(fn, "bmp")) || (strstr(fn, "BMP")))
        {
          ...
          Show_Image();
          wait_user_input = 1;
          lcd_display_string_line( LCD_PIXEL_HEIGHT - 12, "Press any key to continue...             ");
          lcd_set_text_color(LCD_LOG_DEFAULT_COLOR); 
...
//          STM_EVAL_LEDOff(LED3);
//          STM_EVAL_LEDOff(LED4);
          lcd_clear_line( LCD_PIXEL_HEIGHT - 12);
          f_close(&file);
          wait_user_input = 0; 
