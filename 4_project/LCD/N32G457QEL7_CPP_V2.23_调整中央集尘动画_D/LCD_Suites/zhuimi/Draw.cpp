...
//		}
	...
	else if((Message.remainingSecond == 0xEF || level <= 10 )&&(!Message.errorData))//有故障触发则不触发电量低
	{//电量低
...
{	
	if(Message.workState == WorkState_Work)
	{//仅工作状态需要灰尘等级 并判断灰尘等级
...
{
	if(Message.filterReset > FILTER_RESET_RESET)
	{
		FlagContainers.bit.DirtyDta = 1;
		return state;
	}
...
DrawMainShowState Draw::CheckFilterServiceLife(DrawMainShowState state)
{
	...
	{//滤芯寿命提示界面
		...
		else if(Message.filterServiceLife > FILTER_SERVICE_LIFE_EXPIRE)//超出范围的值
		{
			FlagContainers.bit.DirtyDta = 1;
			return state;
		}
...
DrawMainShowState Draw::CheckHighAltitudeMode(DrawMainShowState state)
{
	...
	{
		if(Message.highAltitudeMode == HIGH_ALTITUDE_MODE_ENTERING || Message.highAltitudeMode == HIGH_ALTITUDE_MODE_ENTERED)
		{
		...
		else if(Message.highAltitudeMode == HIGH_ALTITUDE_MODE_EXITING || Message.highAltitudeMode == HIGH_ALTITUDE_MODE_EXITED)
		{
