...
/**
  * @brief  Disable I2C Packet error checking byte
  * @note   This bit is disable by software, and cleared by hardware when the PEC is transferred,
  *         or when a STOP condition or an Address matched is received, also when PE=0.
  *         Writing 0 to this bit has no effet. This bit has no effect when RELOAD is set,
  *         and when SBC is 0 in the slave mode.
  *         If the SMBus feature is not supported, this bit is reserved and forced by hardware to 0.
...
{
    CLEAR_BIT(I2Cx->CON2, I2C_CON2_PECBYTE);
}
...
void        md_i2c_master_send(I2C_TypeDef *I2Cx, uint16_t Nbyte, uint32_t addr10, uint16_t DevAddr, uint8_t *txbuf);
void        md_i2c_master_receive(I2C_TypeDef *I2Cx, uint16_t Nbyte, uint32_t addr10, uint16_t DevAddr, uint8_t *rxbuf);
void        md_i2c_slave_send(I2C_TypeDef *I2Cx, uint16_t Num, uint8_t *txbuf);
void        md_i2c_slave_receive(I2C_TypeDef *I2Cx, uint16_t Num, uint8_t *rxbuf);
void        md_i2c_master_setting(I2C_TypeDef *I2Cx, uint16_t Nbyte, uint32_t addr10, uint16_t DevAddr, uint8_t WriteRead);
/**
  * @}
  */
