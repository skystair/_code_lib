...
/* Private variable -----------------------------------------------------------*/
unsigned char ADCi;
unsigned char u8ADCsampletime;
unsigned char u8ADCsampleflag;
...
/* ADC channel length config */
	...
	adc_regular_channel_config(4, ADC_CHANNEL_5, ADC_SAMPLETIME_13POINT5);
	adc_regular_channel_config(5, ADC_CHANNEL_6, ADC_SAMPLETIME_13POINT5);
	adc_regular_channel_config(6, ADC_CHANNEL_7, ADC_SAMPLETIME_13POINT5);
	adc_regular_channel_config(7, ADC_CHANNEL_8, ADC_SAMPLETIME_13POINT5);
	adc_regular_channel_config(8, ADC_CHANNEL_9, ADC_SAMPLETIME_13POINT5);
	
    /* ADC external trigger enable */
    adc_external_trigger_config(ADC_REGULAR_CHANNEL, ENABLE);
//    /* ADC external trigger source config */
    adc_discontinuous_mode_config(ADC_REGULAR_CHANNEL, 3);//1~8
...
void ADCxfunc(void){
	if(u8ADCsampletime >= 1){
//		if(u8ADCsampleflag == 1)
		{
			u8ADCsampletime = 0;
			u8ADCsampleflag = 0;
//			adc_software_trigger_enable(ADC_REGULAR_CHANNEL);
