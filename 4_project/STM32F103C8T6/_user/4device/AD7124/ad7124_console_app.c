...
struct ad7124_dev * pAd7124_dev = NULL;
...
void func_adc(struct ad7124_dev *device);
...
void func_adc(struct ad7124_dev *device){
	if(ad7124_wait_for_conv_ready(device,100) >=0 ){
		u8Ch = (ad7124_register_map[AD7124_Status].value); 
		ad7124_read_data(device, &s32AdCode[u8Ch]); 
		
		Vad[u8Ch] = (float)s32AdCode[u8Ch] / 0xFFFFFF *3.3;
	}
}
